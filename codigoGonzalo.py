...
class MPChurn_EMPChurn(object):
    ...
    def __init__(self, y_true, y_score=None, alpha=6, beta=14, clv=200, d=10, f=1, case_label=1,):
    ...
    def empc(self, y_score=None):
        ...
        if not self._gamma_needs_update and self._empc:
            ...
            return self._opt_threshold, self._mpc, self._mpcFrac, self._empc
        else:
            # Gamma has been changed, empc needs to be recomputed
            assert self._gamma_needs_update
            self._opt_threshold, self._mpc, self._mpcFrac, self._empc = self._compute_empc()
            return self._opt_threshold, self._mpc, self._mpcFrac, self._empc
    ...
    def _compute_empc(self):
        ...
        indE =  np.where(self._gamma < self._egamma)[0].max()


        fpr, tpr, thresholds = roc_curve(self._yt, self._ys) #
        opt_threshold = thresholds[np.intersect1d(np.where(fpr==self._F1[indE])[0],
                                                  np.where(tpr==self._F0[indE])[0])[0]]

        mpc = self.clv * ((self._egamma * (1-self._delta)-self._phi) * self.pi0*self._F0[indE] - (self._delta+self._phi)*self.pi1*self._F1[indE])
        mpcFrac = self.pi0*self._F0[indE] + self.pi1*self._F1[indE]
        ...
        return opt_threshold, mpc, mpcFrac, empc


    def _compute_gamma(self):
        self._emp_roc_info()
        numerator = self.pi1 * (self._delta + self._phi) * np.diff(self._F1) + self.pi0 * self._phi * np.diff(self._F0)
...
def ejecutar_segmentacion(y_preds, CLV_promedio, y,CLV, alpha,beta,d,f, test_idx, case_label=0, segmentos = 2): #Churners have a ``cases_label`` value.

    predicciones_gonzalo = {}
    for nombre, (_,y_score) in y_preds.items():

        clvscore = pd.DataFrame({'CLV': CLV[test_idx],
                                 'score': y_score,
                                 'classes': y[test_idx],
                                 'ranking': np.argsort(-CLV[test_idx])})
        bk = np.quantile(clvscore['ranking'], np.linspace(0, 1, segmentos+1))
        decilesclv = clvscore.copy()
        decilesclv['decile'] = pd.cut(clvscore['ranking'], bins=bk, labels=False, include_lowest=True)+1
        decilesclv['y_pred_label'] = None

        promedioclv = decilesclv.groupby('decile')['CLV'].mean()

        for i in range(1, segmentos+1):
            scores_i = decilesclv[decilesclv['decile'] == i]['score'].values
            classes_i = decilesclv[decilesclv['decile'] == i]['classes'].values

            calculo = MPChurn_EMPChurn(y_true=classes_i, y_score=scores_i, alpha=alpha, beta=beta, clv=promedioclv[i], d=d, f=f, case_label=case_label)
            opt_threshold, mpc, mpcFrac, empc = calculo.empc()

            y_pred_label_i = np.array((scores_i<opt_threshold), dtype=float)
            decilesclv['y_pred_label'][decilesclv['decile'] == i] = y_pred_label_i

            #mpcFrac is the fraction of the customer base which should be targeted to maximize thre profit generated by the retention campaign
            #z_pred = 1-y_pred_label_i
            #from math import isclose
            #isclose( np.sum(z_pred)/len(z_pred) , mpcFrac )
            
        predicciones_gonzalo[nombre+'_segmentacion'] = np.array(decilesclv['y_pred_label'])

    return predicciones_gonzalo