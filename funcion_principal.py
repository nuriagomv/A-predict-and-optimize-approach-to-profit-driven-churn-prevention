...
def ejecutar(value):
    ...
    X,y,CLV, train_idx, valid_idx, hidden_size, batch_size, epochs, lr, seed, f, d, gamma, loss_flag = value
    ...
    """
    dataset = torch.utils.data.TensorDataset(X_train, y_train, CLV_train)
    train_loader = torch.utils.data.DataLoader(dataset,
                                               sampler = BalancedBatchSampler(dataset, y_train),
...
def entrenar_y_validar(X,y,CLV, f,d,gamma, train_valid_idx, test_idx):
                ...
                for batch_size in tune_batch_size:
                    for hidden_size in tune_hidden_size:
                        for loss_flag in try_loss_flags:
                            values.append( (X,y,CLV, train_idx, valid_idx, hidden_size, batch_size, epochs, lr, seed, f, d, gamma, loss_flag) )

        results = Parallel(n_jobs=num_cores)(delayed(ejecutar)(value) for value in values)
    ...
    z_pred = 1 - torch.relu( torch.sign(y_pred - m) )
        

    """
    resultados = {'d': d,
                  'diccionario_validaciones': diccionario_validaciones, 'best_hyperparams': best_hyperparams, 'best_seed': best_seed,
                  'y_pred_test':y_pred, 'm_test': m, 'z_pred_test': z_pred}
    with open('resultados_d'+str(d)+'.pkl', 'wb') as handle:
        pickle.dump(resultados, handle, protocol=pickle.HIGHEST_PROTOCOL)
    #with open('resultados_d4.36.pkl', 'rb') as handle:
